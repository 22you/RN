        ...
        return ( 
            <View style={{paddingLeft:10,marginTop:5}}>
                ...
                <View style={{flexDirection:'row'}}>
                    ...
                    <TouchableOpacity  onPress={()=>{this.setState({departmentName:item.text})}}><Text style={{color:this.state.departmentName==item.text?'red':'#555'}} >{item.text}</Text></TouchableOpacity>
                </View>
  ...
  render() {
    ...
    return (
      <ScrollView style={{paddingHorizontal:10,paddingVertical:20}}>
         {this.menu(arr)}
         <Button style={{marginVertical:20}} title='确认' type='primary' onPress={() => Toast.info('您选择的是：'+this.state.departmentName)} />
      </ScrollView>
    );
  }
}
